...
describe( "querystring", function () {
  ...
  var complexQueryString = "chicken=tasty&duck=delicious&key%20has%20a%20space=%7B%22value%22%3A%22is%20an%20object%22%2C%22deep%22%3A%7B%22values%22%3A%22are%20ok%22%7D%7D&number=-36.2&array=%5B%22one%22%2C2%2C%223%22%5D&boolean=false&date=2013-12-11T23%3A00%3A00.000Z",
    testObject = {
      chicken: "tasty",
      duck: "delicious",
      "key has a space": {
  ...
  it( "should stringify a complex object to a querystring", function () {
    ...
    var stringifiedQuerystring = querystring.stringify( testObject );
  ...
  it( "should return an empty object if the querystring is invalid", function () {
    ...
    should( querystring.parse( "" ) ).be.empty;
    should( querystring.parse( "=" ) ).be.empty;
    should( querystring.parse( /f/ ) ).be.empty;
    should( querystring.parse( null ) ).be.empty;
    should( querystring.parse( undefined ) ).be.empty;
    should( querystring.parse( [] ) ).be.empty;
    should( querystring.parse( {} ) ).be.empty;
